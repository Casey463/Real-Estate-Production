...
## Docker compose front and backend together. 
Make sure both frontend and backend are in the same directory, seperate folders.
Backend lower level docker-compose will still function as normal. 
Create docker-compose.yml at this higher level directory and add the below code:

```services:
  backend:
    build: ./real-estate-backend-remade
    volumes:
      - ./real-estate-backend-remade:/app
    ports:
      - "8000:8000"

  frontend:
    build: ./real-estate-dash-remade
    ports:
      - "3001:3000"
    environment:
